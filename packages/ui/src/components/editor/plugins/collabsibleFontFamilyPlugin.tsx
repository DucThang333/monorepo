import { SelectValue } from '@package/ui/components/select';
...
function CollabsibleFontFamilyPlugin(props: CollabsibleFontFamilyPluginProps) {
  ...
  const { fontFamilyKey } = useEditorState({
    editor,
    selector: ({ editor: editorS }) => ({
      fontFamilyKey: getActiveKeyFontFamily(editorS),
    }),
    equalityFn: (a, b) => a.fontFamilyKey === b.fontFamilyKey,
  });
  return (
    <EditorCollabsible
...
function getActiveKeyFontFamily(editor: Editor) {
